// Lightweight UI language handler (DE/EN) persisted via /api/ui-language
(function () {
  let currentLang = 'DE';
  const de2en = new Map([
  ...
  async function fetchLanguage() {
    try {
      const res = await fetch('/api/ui-language', { cache: 'no-store' });
      if (!res.ok) throw new Error('Failed to load language');
      const data = await res.json();
      return (data.language || 'DE').toUpperCase();
    } catch (e) {
  ...
  async function initLanguageDropdown() {
    const select = document.getElementById('ui-language-select');
    if (!select) return;
    const current = await fetchLanguage();
