...
"""
    for attempt in range(MAX_ATTEMPTS):
        ...
        if answer == "JA":
            print("➡️ Ticket ist relevant/hilfreich.")
            return True
        if answer == "NEIN":
            print("➡️ Ticket nicht hilfreich – wird übersprungen.")
            return False
        print(f"⚠️ Ungültige LLM-Antwort (Versuch {attempt+1}): '{answer}' – wiederhole...")
        time.sleep(1)
    print(f"❌ Konnte keine gültige JA/NEIN-Antwort vom LLM bekommen. Ticket wird übersprungen.")
    return False
...
"""
    ...
    for attempt in range(MAX_ATTEMPTS):
        ...
        if not answer:
            print("❌ Keine Antwort vom LLM – Ticket wird übersprungen.")
            return None
        cleaned_answer = clean_llm_json_answer(answer)
        try:
            fields = json.loads(cleaned_answer)
                    # 🛠️ Feldnamen normalisieren (z. B. "lösung" → "loesung")
            if "lösung" in fields:
                fields["loesung"] = fields.pop("lösung")
            if "kürzel" in fields:
                fields["kurzbeschreibung"] = fields.pop("kürzel")
            if "tag" in fields and isinstance(fields["tag"], list):
                fields["tags"] = fields.pop("tag")
            # Prüfe nur, ob alle Keys vorhanden sind – egal ob null oder Wert
            if (
        ...
        time.sleep(1)
    print(f"❌ Konnte keine gültige JSON-Antwort vom LLM bekommen. Ticket wird übersprungen.")
    return None
...
def process_and_store_ticket(ticket, point_id):
    ...
    loesg = f"Lösung: {fields.get('loesung', '').strip()}"
